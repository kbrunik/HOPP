...
def optimize_plant_AEP(x, scenario, print_status=False):

    nTurbs = int(len(x)/2)

    scenario.systems['Wind']['Windpower'].Farm.wind_farm_xCoordinates = x[0:nTurbs].tolist()
    scenario.systems['Wind']['Windpower'].Farm.wind_farm_yCoordinates = x[nTurbs:].tolist()
...
def layout_opt(x0, scenario, verts, print_status=False):
    ...
    cons = []
    tmp1 = {'type': 'ineq', 'fun': lambda x: func_tools.spaceConstraint(x, print_status) - min_spacing}
    tmp2 = {'type': 'ineq', 'fun': lambda x, *args: func_tools.distance_from_polygon_con(x, verts, print_status), 'args': (verts,)}
    cons.append(tmp1)
    cons.append(tmp2)
    ...
    # optimize the layout
    resPlant = minimize(optimize_plant_AEP, x0, args=(scenario, ), method='COBYLA', constraints=cons,
...
def solar_opt(x0,nArrays,verts,x,y):
    ...
    # 3. (mixed integer - later) - number of sections - the above number of rows and number in rows are integers -> using length and width as a proxy
    ...
    # constraints
    cons = []
    tmp1 = {'type': 'ineq','fun' : lambda x,*args: no_obstacles(x,verts,nArrays,turb_locs)    , 'args':(verts,nArrays,turb_locs)}
