...
class CspDispatch(Dispatch):
    ...
    def _create_receiver_constraints(csp):
        ...
        csp.receiver_startup_inventory_balance = pyomo.Constraint(
            ...
            expr=(
                csp.receiver_startup_inventory
                <= (
        ...
        csp.receiver_operation_startup = pyomo.Constraint(
            ...
            expr=(
                ...
                <= (
                    csp.receiver_startup_inventory
                    / csp.receiver_required_startup_energy
                )
    ...
    def _create_storage_linking_constraints(self):
        ...
        def tes_linking_rule(m, t):
            if t == self.blocks.index_set().first():
                return self.blocks[t].previous_thermal_energy_storage \
    ...
    def _create_receiver_linking_constraints(self):
        ...
        def receiver_startup_inventory_linking_rule(m, t):
            if t == self.blocks.index_set().first():
                return self.blocks[t].previous_receiver_startup_inventory \
    ...
    def _create_cycle_linking_constraints(self):
        ...
        def cycle_startup_inventory_linking_rule(m, t):
            if t == self.blocks.index_set().first():
                return self.blocks[t].previous_cycle_startup_inventory \
        ...
        def cycle_thermal_power_linking_rule(m, t):
            if t == self.blocks.index_set().first():
                return self.blocks[t].previous_cycle_thermal_power \
    ...
    def initialize_parameters(self):
        ...
        self.cost_per_cycle_start = (
            self.objective_cost_terms['cost_per_cycle_start_rel'] * csp.value('P_ref')
        )
        self.cost_per_change_thermal_input = (
            self.objective_cost_terms['cost_per_change_thermal_input']
        )
    ...
    def set_part_load_cycle_parameters(self):
        ...
        if 'cycle_eff_load_table' in tables:
            ...
            norm_heat_pts = [
                tables['cycle_eff_load_table'][i][0] / q_pb_design for i in range(num_pts)
            ]  # Load fraction
            efficiency_pts = [
            ...
            self.maximum_cycle_power = (
                self.maximum_cycle_thermal_power * self._system_model.cycle_nominal_efficiency
            )
    ...
    def set_ambient_temperature_cycle_parameters(self, dry_bulb_temperature):
        ...
        if 'cycle_eff_Tdb_table' in tables:
            ...
            efficiency_pts = [
                ...
                * self._system_model.cycle_nominal_efficiency for i in range(nT)
            ]  # Efficiency
            # Fraction of cycle design gross output consumed by cooling
            wcondfpts = [tables['cycle_wcond_Tdb_table'][i][1] for i in range(nT)]
            ...
            # Tables not returned from ssc, but can be taken from user-defined cycle inputs
            D = self.interpret_user_defined_cycle_data(tables['ud_ind_od'])
            # first index in udpc data corresponding to performance at design
            # point HTF T, and design point mass flow
            k = 3 * D['nT'] + 3 * D['nm'] + D['nTamb']  
            npts = D['nTamb']
            efficiency_pts = [
                self._system_model.cycle_nominal_efficiency
            ...
            wcondfpts = [
                (self._system_model.value('ud_f_W_dot_cool_des') / 100.)
            ...
            print(
                "WARNING: Dispatch optimization cycle ambient temperature corrections are "
                "not set up."
            )
            n = len(dry_bulb_temperature)
            self.cycle_ambient_efficiency_correction = (
                [self._system_model.cycle_nominal_efficiency] * n
            )
        ...
        for i in range(len(field_gen)):
            su_fraction[i] = min(
                ...
                max(
                    ...
                    self.receiver_required_startup_energy / max(
                        1e-6, field_gen[i] * time_duration[i]
                    )
                )
            )
    ...
    def update_initial_conditions(self):
        ...
        if (
            ...
            if (
                ...
                > (1.0 - 1.e-6) * self.cycle_required_startup_energy
            ):
                self.initial_cycle_startup_inventory = self.cycle_required_startup_energy
